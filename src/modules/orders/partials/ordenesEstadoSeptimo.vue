<template>
    <div :id="idi" class="p-3">
        ...
        <div class="ms-5">
            <div v-for="detalle in detalleDeCobroPrecio" class="d-middle-bt mb-2">
                <p class="f-light f-14">{{ detalle.producto }}</p>
        ...
        <div class="d-flex flex-column gap-3 ms-5 mt-3 w-266px">
            ...
            <template v-for="(comprobante, i) in comprobantePagoVerificar" :key="i">
                <div class="d-flex gap-2 p-3 br-14" :class="[ comprobante.verificar === 'Confirmado' ? 'border-green-clear': 'border-red' ]">
                    <div class="d-middle-center wh-18">
                        <i class="f-24" :class="[ comprobante.verificar === 'Confirmado' ? 'icon-verify text-green-clear': 'icon-close text-red' ]" />
                    </div>
                    <div>
                        <p class="f-medium f-12">{{ comprobante.verificar }}: <span class="f-light"> {{ comprobante.date }} - {{ comprobante.time }} </span></p>
                        <p class="f-medium f-14">{{ comprobante.responsable }}</p>
                    </div>
                </div>
            </template>
        </div>
    </div>
    <modal ref="refModalConfirmarTransferencia" :modal-width="401" modal-title="Confirmar transferencia" action="Confirmar">
        <div class="px-5 py-4">
            <p class="text-grey-light f-15">¿Confirmar que el cliente realizó la transferencia de pago de la orden?</p>
        </div>
    </modal>
    <modal ref="refModalRechazarTransferencia" :modal-width="401" modal-title="Rechazar transferencia" action="Rechazar">
        <div>
...
const detalleDeCobroPrecio = ref([
    { producto: 'Productos (5)', precio: '79500' },
    { producto: 'Toppings', precio: '8500' },
    { producto: 'Descuento promo', precio: '11200' },
    { producto: 'Domicilio', precio: 'Gratis' },
])
...
const comprobantePagoVerificar = ref([
    { verificar: 'Confirmado', date: '15 Nov.', time: '10:45 a.m', responsable: 'Juliana Carreño' },
    { verificar: 'Rechazado', date: '15 Nov.', time: '10:45 a.m', responsable: 'Juliana Carreño' },
])
